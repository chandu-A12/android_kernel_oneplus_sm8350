...
void __attribute__((weak)) op_switch_normal_set(void)
{
	return;
}

void __attribute__((weak)) smblib_bypass_vsafe0_control(struct smb_charger *chg, bool enable)
{
	return;
}
/*----For FCC/jeita----------------------------------------*/
				   ...
				   union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_VOLTAGE_NOW:
		if (chip->wireless_mode == WIRELESS_MODE_RX)
			val->intval = wlchg_wireless_get_vout();
		else
			val->intval = 0;
		break;
	case POWER_SUPPLY_PROP_VOLTAGE_MAX:
		if (chip->wireless_mode == WIRELESS_MODE_RX)
			val->intval = chip->wlchg_status.target_vol;
		else
			val->intval = 0;
		break;
	case POWER_SUPPLY_PROP_CURRENT_NOW:
		if (chip->wireless_mode == WIRELESS_MODE_RX)
			val->intval = g_rx_chip->chg_data.iout;
		else
			val->intval = 0;
		break;
	case POWER_SUPPLY_PROP_CURRENT_MAX:
		if (chip->wireless_mode == WIRELESS_MODE_RX)
			val->intval = chip->wlchg_status.max_current;
		else
			val->intval = 0;
		break;
	...
	case POWER_SUPPLY_PROP_REAL_TYPE:
		...
		case ADAPTER_TYPE_UNKNOWN:
		{
			...
			if (wlchg_run_mode == RX_RUNNING_MODE_BPP)
				val->intval = POWER_SUPPLY_TYPE_USB_DCP;
			else if (wlchg_run_mode == RX_RUNNING_MODE_EPP)
...
#ifdef OPLUS_CUSTOM_OP_DEF
		smblib_bypass_vsafe0_control(normal_charger, true);
#endif
...
#ifdef OPLUS_CUSTOM_OP_DEF
		smblib_bypass_vsafe0_control(normal_charger, false);
#endif
	}
...
/*		if (get_prop_fast_chg_started(normal_charger)) {
		...
		smblib_usb_plugin_locked(normal_charger);
		msleep(100);
						    ...
						    size_t count, loff_t *ppos)
{
	...
	char page[10];
	int rx_adc_result = 0;
	struct op_chg_chip *wlchg_dev = PDE_DATA(file_inode(file));

	if (wlchg_dev == NULL) {
		pr_err("wlchg dev is not found\n");
		return -ENODEV;
	}

	if (wlchg_dev->wlchg_status.rx_adc_test_pass == true) {
		rx_adc_result = 1;
	} else {
		rx_adc_result = 0;
	}
						     ...
						     size_t len, loff_t *lo)
{
	...
	if (rx_adc_cmd == 0) {
		...
		wlchg_dev->wlchg_status.rx_adc_test_enable = false;
	} else if (rx_adc_cmd == 1) {
		pr_err("rx_adc_test: set 1.\n");
		wlchg_dev->wlchg_status.rx_adc_test_enable = true;
	} else {
...
static const struct file_operations oplus_chg_wlchg_proc_idt_adc_test_ops = {
	.read = oplus_chg_wlchg_proc_idt_adc_test_read,
	.write = oplus_chg_wlchg_proc_idt_adc_test_write,
	.open = simple_open,
	.owner = THIS_MODULE,
};
...
bool oplus_wpc_get_ffc_charging(void)
{
	...
	if (g_op_chip->wireless_mode == WIRELESS_MODE_RX) {
		if (g_op_chip->wlchg_status.wpc_ffc_charge) {
			return true;
		} else {
			return false;
		}
...
void oplus_wpc_set_rtx_function(bool is_on)
{
...
{
	if (!g_op_chip) {
		chg_err("g_op_chip null, return\n");
		return true;
	}
	return false;
}

void oplus_wpc_wpc_print_log(void)
{
	return;
}

bool __attribute__((weak)) oplus_wpc_get_normal_charging(void)
{
	return false;
}
...
struct oplus_wpc_operations p922x_ops = {
	.wpc_get_wireless_charge_start = oplus_wpc_get_wireless_charge_start,
	.wpc_get_fast_charging = oplus_wpc_get_fast_charging,
	.wpc_get_otg_charging = oplus_wpc_get_otg_charging,
	.wpc_get_ffc_charging = oplus_wpc_get_ffc_charging,
	.wpc_get_fw_updating = oplus_wpc_get_fw_updating,
	.wpc_get_adapter_type = oplus_wpc_get_adapter_type,
	.wpc_set_vbat_en = oplus_wpc_set_vbat_en_val,
	.wpc_set_booster_en = oplus_wpc_set_booster_en_val,
	.wpc_set_ext1_wired_otg_en = oplus_wpc_set_ext1_wired_otg_en_val,
	.wpc_set_ext2_wireless_otg_en = oplus_wpc_set_ext2_wireless_otg_en_val,
	.wpc_set_rtx_function = oplus_wpc_set_rtx_function,
	.wpc_set_rtx_function_prepare = oplus_wpc_set_rtx_function_prepare,
	.wpc_print_log = oplus_wpc_wpc_print_log,
};
/*--------------------For OPLUG Interface------------------*/
...
static int wlchg_driver_probe(struct platform_device *pdev)
{
	...
	ret = of_property_read_u32(chip->dev->of_node, "op,wls-power", &chip->wls_prj_power);
	if (ret < 0) {
		pr_err("op,wls-power reading failed, ret=%d\n", ret);
		chip->wls_prj_power = 0;
	}
	chg_info("support %d W power\n", chip->wls_prj_power);
