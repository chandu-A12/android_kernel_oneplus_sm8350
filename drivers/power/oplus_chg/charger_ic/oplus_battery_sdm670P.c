...
static bool usbtemp_flag = false;
...
#ifdef VENDOR_EDIT//Fanhong.Kong@ProDrv.CHG,add 2018/06/02 for SWARP OTG
		if(chip->vbatt_num == 2)
		{
...
void smblib_usb_plugin_locked(struct smb_charger *chg)
{
	...
	if(oplus_warp_get_fastchg_started() == true && oplus_warp_get_adapter_update_status() != 1){
...
#ifdef VENDOR_EDIT//Fanhong.Kong@ProDrv.CHG,add 2018/06/02 for SWARP OTG
	//if (chip->vbatt_num == 2)
		//vote(chg->apsd_disable_votable, SWARP_OTG_VOTER, false, 0);
#endif/*VENDOR_EDIT*/
...
static void oplus_ccdetect_work(struct work_struct *work)
{
	...
	if (level != 1) {
		...
		if (typec_mode == POWER_SUPPLY_TYPEC_NONE) {
			smblib_err(chg, "oplus_ccdetect_work typec_mode=%d\n", typec_mode);
		...
		oplus_set_dpdm_status(POWER_SUPPLY_TYPEC_PLUGIN);
		if (divider_in_auto_mode == false || (oplus_warp_get_allow_reading() == false)) {
			cancel_delayed_work_sync(&chg->divider_set_work);
...
static void oplus_get_usbtemp_volt(struct oplus_chg_chip *chip)
{
	...
	if (!chip->pmic_spmi.usb_temp_v_chan ||
		IS_ERR(chip->pmic_spmi.usb_temp_v_chan) ||
		PTR_ERR(chip->pmic_spmi.usb_temp_v_chan) == -EPROBE_DEFER) {
		chg_err("usb_temp_v_chan NULL\n");
		chip->usbtemp_volt_l = chip->usbtemp_volt_r;
		goto out;
	}

	rc = iio_read_channel_processed(chip->pmic_spmi.usb_temp_v_chan , &val);
	chip->usbtemp_volt_l = val;
...
static void get_usb_temp(struct oplus_chg_chip *chip)
{
	...
	for (i = ARRAY_SIZE(con_volt_18383) - 1; i >= 0; i--) {
		if (con_volt_18383[i] >= chip->usbtemp_volt_l)
			break;
		else if (i == 0)
			break;
	}
	chip->usb_temp_l = con_temp_18383[i]+1;
...
static void oplus_set_usb_status(void)
{
	return;
}
...
static int oplus_usbtemp_monitor_main(void *data)
{
	...
	while (!kthread_should_stop() && dischg_flag == false) {
		...
		if(oplus_get_otg_online_status() != 1 && oplus_chg_is_usb_present() == false){
			count = 0;
			msleep(delay);
			wait_event_interruptible(oplus_usbtemp_wq, (oplus_chg_is_usb_present() == true || chg->typec_present == true));
		}else{
...
#ifdef CONFIG_HIGH_TEMP_VERSION
							pinctrl_select_state(chip->normalchg_gpio.pinctrl, chip->normalchg_gpio.dischg_disable);
							oplus_set_usb_status();
#else
							rc = smblib_masked_write(chg, TYPE_C_INTRPT_ENB_SOFTWARE_CTRL_REG,
									TYPEC_POWER_ROLE_CMD_MASK, UFP_EN_CMD_BIT);//bit[2:0]=0x4
							if (rc < 0) {
								chg_err("[OPLUS_CHG][%s]: Couldn't set 0x1368[2] rc=%d\n", __func__, rc);
							}
							rc = smblib_masked_write(chg, TYPE_C_INTRPT_ENB_SOFTWARE_CTRL_REG,
									TYPEC_DISABLE_CMD_BIT | UFP_EN_CMD_BIT,
									TYPEC_DISABLE_CMD_BIT);
							if (rc < 0) {
								chg_err("[OPLUS_CHG][%s]: Couldn't set 0x1368[0] rc=%d\n", __func__, rc);
							}
							...
							if (count_r >= retry_cnt || count_l >= retry_cnt) {
								if (!IS_ERR_OR_NULL(chip->normalchg_gpio.dischg_enable)) {
									...
									rc = smblib_masked_write(chg, TYPE_C_INTRPT_ENB_SOFTWARE_CTRL_REG,
											...
											TYPEC_DISABLE_CMD_BIT);
									if (rc < 0) {
										chg_err("Couldn't set 0x1368[0] rc=%d\n", rc);
									}
									oplus_set_usb_status(USB_TEMP_HIGH);
									chip->chg_ops->charger_suspend();
									usbtemp_flag  = true;
...
static void oplus_set_otg_switch_status(bool value)
{
	...
	if(usb_status == 1){
		printk(KERN_ERR "[OPLUS_CHG][%s]: usbtemp is hight!\n", __func__);
		return;
	}
	chg = &chip->pmic_spmi.smb2_chip->chg;
		...
		union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_FAST_CHG_TYPE:
		if(((oplus_warp_get_fastchg_started() == true) ||
			...
			(g_oplus_chip->vbatt_num == 1)&&(oplus_warp_get_fast_chg_type() == FASTCHG_CHARGER_TYPE_UNKOWN)){
			val->intval = CHARGER_SUBTYPE_FASTCHG_WARP;
		}else{
			val->intval = oplus_warp_get_fast_chg_type();
		}
        break;
#endif /*VENDOR_EDIT*/
#ifdef VENDOR_EDIT
...
#ifdef VENDOR_EDIT//Fanhong.Kong@ProDrv.CHG,add 2018/04/20 for SWARP
...
static int smb2_pm_resume(struct device *dev)
{
	...
	if (!g_oplus_chip)
		return 0;
    if (oplus_usbtemp_check_is_support() == true) {
        if (IS_ERR_OR_NULL(g_oplus_chip->normalchg_gpio.usb_temp_adc)) {
            chg_err("get usb_temp_adc fail\n");
            return -EINVAL;
        } else {
            pinctrl_select_state(g_oplus_chip->normalchg_gpio.pinctrl, g_oplus_chip->normalchg_gpio.usb_temp_adc);
        }
    }
...
static int smb2_pm_suspend(struct device *dev)
{
	if (!g_oplus_chip)
		return 0;
    if (oplus_usbtemp_check_is_support() == true) {
        if (IS_ERR_OR_NULL(g_oplus_chip->normalchg_gpio.usb_temp_adc_suspend)) {
            chg_err("get usb_temp_adc_suspend fail\n");
            return -EINVAL;
        } else {
            pinctrl_select_state(g_oplus_chip->normalchg_gpio.pinctrl, g_oplus_chip->normalchg_gpio.usb_temp_adc_suspend);
        }
    }
...
static int smb2_probe(struct platform_device *pdev)
{
	...
	if (oplus_chip->vbatt_num == 1) {
		...
		if (oplus_gauge_ic_chip_is_null() || oplus_warp_check_chip_is_null()
				|| oplus_charger_ic_chip_is_null() || oplus_adapter_check_chip_is_null()) {
			chg_err("[oplus_chg_init] warp || gauge || chg not ready, will do after bettery init.\n");
			return -EPROBE_DEFER;
		}
		oplus_chip->chg_ops = (oplus_get_chg_ops());
	}

	chg_err("SMB2_Probe Start----\n");
#endif
    ...
    g_smb_chip = &oplus_chip->pmic_spmi.smb2_chip->chg;
		if (!g_smb_chip) {
				chg_err("smb2_probe chip not ready\n");
		}
        if (!g_oplus_chip) {
				chg_err("smb2_probe chip not ready1\n");
		}
#ifdef VENDOR_EDIT
...
#endif
#ifdef VENDOR_EDIT
	oplus_chip->pmic_spmi.usb_temp_v_chan = iio_channel_get(oplus_chip->dev, "usb_temp_v");
	if (IS_ERR(oplus_chip->pmic_spmi.usb_temp_v_chan))
		chg_err("Couldn't get usb_temp_v_chan...\n");
		//return PTR_ERR(oplus_chip->pmic_spmi.usb_temp_v_chan);    
#endif
...
#ifdef VENDOR_EDIT
void get_battery_type(int type)
{
    if (g_oplus_chip != NULL) {
        g_oplus_chip->authenticate = type;
        printk("g_oplus_chip->authenticate =%d\n",g_oplus_chip->authenticate);
    }
}
#endif /* VENDOR_EDIT */
