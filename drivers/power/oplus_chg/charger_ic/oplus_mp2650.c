...
int mp2650_input_current_limit_ctrl_by_warp_write(int current_ma)
{
	int rc = 0;
	int tmp = 0;
	int count =0;
	...
	chg_err("set charge ctrl_by_warp current limit = %d\n", current_ma);
...
int mp2650_reset_charger(void)
{
	...
	if(chip->probe_flag){
		...
		rc = mp2650_config_interface(REG10_MP2650_ADDRESS, 0x07, 0xFF);
		rc = mp2650_config_interface(REG11_MP2650_ADDRESS, 0x0E, 0xFF);
		rc = mp2650_config_interface(REG12_MP2650_ADDRESS, 0x73, 0xFF);
		rc = mp2650_config_interface(REG31_MP2650_ADDRESS, 0x03, 0xFF);
		rc = mp2650_config_interface(REG33_MP2650_ADDRESS, 0x15, 0xFF);
		rc = mp2650_config_interface(REG36_MP2650_ADDRESS, 0xB5, 0xFF);
...
struct oplus_chg_operations  mp2650_chg_ops = {
    ...
    .input_current_ctrl_by_warp_write = mp2650_input_current_limit_ctrl_by_warp_write,
