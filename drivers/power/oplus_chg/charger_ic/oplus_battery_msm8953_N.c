...
{
	/* Add for pro 17151 use gpio for usbid by wenbin.liu*/
	if((gpio_is_valid(chip->normalchg_gpio.usbid_gpio)) && chip->pmic_spmi.usbid_gpio_support)
		return true;

	if((gpio_is_valid(chip->normalchg_gpio.usbid_gpio)) && ((get_PCB_Version()== HW_VERSION__13) || (get_PCB_Version()== HW_VERSION__14)))
		return true;
	else
		return false;
}
...
void oplus_usbid_irq_register(struct oplus_chg_chip *chip)
{
	...
	retval = devm_request_threaded_irq(chip->dev, chip->normalchg_gpio.usbid_irq, NULL, 	\
		usbid_change_handler, IRQF_TRIGGER_FALLING | IRQF_TRIGGER_RISING | IRQF_ONESHOT, "usbid-change",	\
			chip);						\
	if (retval < 0) {							\
...
static int smb_parse_dt(struct oplus_chg_chip *chip)
{
	...
	/* add for support usbid use gpio  for 17151, wenbin.liu add in 17/10/16.  should para befor usb id gpio init */
	chip->pmic_spmi.usbid_gpio_support = of_property_read_bool(node, "qcom,usbid-gpio-support");
	...
	chip->normalchg_gpio.usbid_gpio = 
		of_get_named_gpio(chip->pmic_spmi.spmi->dev.of_node, "qcom,usbid-gpio", 0);
	if(chip->normalchg_gpio.usbid_gpio <= 0) {
	...
	/* add for soc restore support */
	chip->pmic_spmi.soc_restore_support = of_property_read_bool(node, "qcom,soc-store-support");
...
void otg_enable_id_value (void) 
{
	...
	if(oplus_usbid_check_is_gpio(the_chip) == true) {
			oplus_set_usbid_active(the_chip);
			usbid_change_handler(0, the_chip);
			chg_err("otg_enable_id_value,usbid_gpio = %d\r\n",gpio_get_value(the_chip->normalchg_gpio.usbid_gpio));
	} else {
...
void otg_disable_id_value (void)
{
	...
	if(oplus_usbid_check_is_gpio(the_chip) == true) {
		//oplus_usbid_eint_unregister(the_chip);
		oplus_set_usbid_sleep(the_chip);
		usbid_change_handler(0, the_chip);
		chg_err("otg_disable_id_value,usbid_gpio = %d\r\n",gpio_get_value(the_chip->normalchg_gpio.usbid_gpio));
