...
static int oplus_chg_get_fv_monitor(struct oplus_chg_chip *chip)
{
	...
	switch(chip->tbatt_status) {
		...
		case BATTERY_STATUS__NORMAL:
			if (oplus_warp_get_fastchg_to_normal() && chip->charging_state != CHARGING_STATUS_FULL)
				default_fv = chip->limits.temp_normal_vfloat_mv_warpchg;
			else
	...
	union power_supply_propval *val)
{
	...
	if (g_oplus_chip->charger_exist) {
		if ((g_oplus_chip->charger_type == POWER_SUPPLY_TYPE_USB_DCP) || (oplus_warp_get_fastchg_started() == true)
		...
		if ((oplus_warp_get_fastchg_started() == true) || (oplus_warp_get_fastchg_to_normal() == true)
			|| (oplus_warp_get_fastchg_to_warm() == true) || (oplus_warp_get_adapter_update_status() == ADAPTER_FW_NEED_UPDATE)
			|| (oplus_warp_get_btb_temp_over() == true) || g_oplus_chip->mmi_fastchg == 0) {
			g_oplus_chip->ac_online = true;
		} else {
			g_oplus_chip->ac_online = false;
		}
...
/* CHARGER_TEMP and CHARGER_TEMP_MAX is dependent on FG and only for HVDCP */
	...
	case POWER_SUPPLY_PROP_CHARGE_TECHNOLOGY:
		if (g_oplus_chip)
			val->intval = g_oplus_chip->warp_project;
		else
			val->intval = true;
		break;
