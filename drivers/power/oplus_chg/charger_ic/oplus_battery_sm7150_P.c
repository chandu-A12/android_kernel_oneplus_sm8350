...
static int opluschg_charge_disble(void);
static int opluschg_suspend(void);
static int opluschg_unsuspend(void);
static void opluschg_set_chargerid_switch(int value);
static int opluschg_get_chargerid_switch(void);
static bool opluschg_is_usb_present(void);
static void opluschg_rerun_aicl(void);
static void opluschg_set_usb_props_type(enum power_supply_type type);
static bool opluschg_get_otg_switch(void);
static bool opluschg_get_otg_online(void);
static void opluschg_set_otg_switch(bool value);
static void opluschg_monitor_func(struct work_struct *work);
static void typec_disable_cmd_work(struct work_struct *work);
static int opluschg_read_adc(int channel);
...
#ifdef OPLUS_FEATURE_CHG_BASIC
/* Yichun.Chen  PSW.BSP.CHG  2019-05-06  for rf/wlan mode suspend usb */
...
/* Yichun.Chen  PSW.BSP.CHG  2019- */
		...
		if (!(apsd_result->pst == POWER_SUPPLY_TYPE_USB_FLOAT &&
				chg->real_charger_type == POWER_SUPPLY_TYPE_USB)) {
			chg->real_charger_type = apsd_result->pst;
			opluschg_set_usb_props_type(apsd_result->pst);
		}
#endif
	}
...
/* Yichun.Chen  PSW.BSP.CHG  2019-04-08  for charge */
		...
		if (oplus_warp_get_fastchg_started() == false && g_oplus_chip) {
			opluschg_set_chargerid_switch(0);
...
/* Yichun.Chen  PSW.BSP.CHG  2019-04-08  for charge */
	if (vbus_rising) {
		wake_up_interruptible(&opluschg_usbtemp_wq);
...
#ifdef OPLUS_FEATURE_CHG_BASIC
/* Yichun.Chen  PSW.BSP.CHG  2019-07-27  for usbtemp detect */
...
/* Yichun.Chen  PSW.BSP.CHG  2019-04-08  for charge */
	if (chg->typec_mode == POWER_SUPPLY_TYPEC_SINK
		...
		//vote(chg->apsd_disable_votable, SWARP_OTG_VOTER, true, 0);
	}
#endif
...
#ifdef OPLUS_FEATURE_CHG_BASIC
/* Yichun.Chen  PSW.BSP.CHG  2019-04-08  for charge */
	INIT_DELAYED_WORK(&chg->opluschg_monitor_work, opluschg_monitor_func);
#endif
...
/* Yichun.Chen  PSW.BSP.CHG  2019-04-08  for charge */
	case POWER_SUPPLY_PROP_OTG_SWITCH:
		val->intval = opluschg_get_otg_switch();
		break;
	case POWER_SUPPLY_PROP_OTG_ONLINE:
		val->intval = opluschg_get_otg_online();
		break;
	case POWER_SUPPLY_PROP_USB_STATUS:
		val->intval = chg->usb_status;
		break;
...
/* Yichun.Chen  PSW.BSP.CHG  2019-04-08  for charge */
	case POWER_SUPPLY_PROP_OTG_SWITCH:
		opluschg_set_otg_switch(!!val->intval);
		break;
#endif
	default:
		...
		union power_supply_propval *val)
{
	...
	switch (psp) {
#ifndef OPLUS_FEATURE_CHG_BASIC
/* Yichun.Chen  PSW.BSP.CHG  2019-04-12  oplus edit power_supply node */
...
/* Yichun.Chen  PSW.BSP.CHG  2019-04-12  oplus edit power_supply node */
	...
	case POWER_SUPPLY_PROP_CHARGE_QNOVO_ENABLE:
		...
		break;
#ifndef OPLUS_FEATURE_CHG_BASIC
/* Yichun.Chen  PSW.BSP.CHG  2019-07-05  jump to oplus_charger.c */
...
#ifdef OPLUS_FEATURE_CHG_BASIC
/* Yichun.Chen	PSW.BSP.CHG  2019-08-17  for disable qg-irq */
...
#ifdef OPLUS_FEATURE_CHG_BASIC
/* Yichun.Chen  PSW.BSP.CHG  2019-06-20  for huawei nonstandard typec cable */
	rc = smblib_masked_write(chg, DEBUG_ACCESS_SNK_CFG_REG, 0xFF, 0x7);
	if (rc < 0)
	    chg_err("fail to write pmic register\n");
#endif
...
static int smb5_show_charger_status(struct smb5 *chip)
{
	struct smb_charger *chg = &chip->chg;
	union power_supply_propval val;
	int usb_present, batt_present, batt_charge_type;
	int rc;
...
static struct delayed_work opluschg_debug_work;
...
static void opluschg_debug_func(struct work_struct *w)
{
	int i, j, k;
	u8 stat[16];
	...
	schedule_delayed_work(&opluschg_debug_work, msecs_to_jiffies(charge_debug_delay > 1000 ? charge_debug_delay : 1000));
}

bool opluschg_pd_sdp = false;
static int opluschg_get_charger_type(void)
{
	u8 apsd_stat;
	int rc;
	struct smb_charger *chg = NULL;
...
static int opluschg_get_fv_monitor(struct oplus_chg_chip *chip)
{
    int default_fv = 0;
...
static int opluschg_get_vbatt_full_vol_sw(struct oplus_chg_chip *chip)
{
	int default_fv = 0;
...
static void opluschg_monitor_func(struct work_struct *work)
{
	...
	if (oplus_warp_get_fastchg_started() == true || chip->charger_volt < 4400) {
		counts = 0;
		goto rerun_work;
	}
	if (chip->tbatt_status == BATTERY_STATUS__COLD_TEMP)
		rechg_vol = opluschg_get_fv_monitor(chip) - 300;
	else if (chip->tbatt_status == BATTERY_STATUS__LITTLE_COLD_TEMP)
		rechg_vol = opluschg_get_fv_monitor(chip) - 200;
	else
		rechg_vol = opluschg_get_fv_monitor(chip) - 100;
	if ((chip->batt_volt > rechg_vol - 10) && chip->batt_full) {
		counts = 0;
		goto rerun_work;
	} else if (chip->batt_volt > opluschg_get_vbatt_full_vol_sw(chip) - 10) {
		counts = 0;
		goto rerun_work;
	}
	...
	if (counts >= (chip->batt_full ? 8 : 3)) {//because rechg counts=6
		...
		if (rc < 0) {
			printk(KERN_ERR "opluschg_monitor_func: Couldn't get BATTERY_CHARGER_STATUS_8_REG status rc=%d\n", rc);
			goto rerun_work;
		}
		if (get_client_vote(chg->usb_icl_votable, BOOST_BACK_VOTER) == 0
				&& get_effective_result(chg->usb_icl_votable) <= USBIN_25MA) {
			printk(KERN_ERR "opluschg_monitor_func: boost back\n");
			if (chg->wa_flags & BOOST_BACK_WA)
				vote(chg->usb_icl_votable, BOOST_BACK_VOTER, false, 0);
		}
		...
		if (stat & PRE_TERM_BIT) {
			...
			printk(KERN_ERR "opluschg_monitor_func: PRE_TERM_BIT is set[0x%x], clear it\n", stat);
			rc = smblib_masked_write(chg, USBIN_CMD_IL_REG, USBIN_SUSPEND_BIT, 1);
			if (rc < 0) {
				printk(KERN_ERR "opluschg_monitor_func: Couldn't set USBIN_SUSPEND_BIT rc=%d\n", rc);
				goto rerun_work;
			}
			msleep(50);
			rc = smblib_masked_write(chg, USBIN_CMD_IL_REG, USBIN_SUSPEND_BIT, 0);
			if (rc < 0) {
				printk(KERN_ERR "opluschg_monitor_func: Couldn't clear USBIN_SUSPEND_BIT rc=%d\n", rc);
				goto rerun_work;
			}
			msleep(10);
			rc = smblib_masked_write(chg, AICL_CMD_REG, RESTART_AICL_BIT, RESTART_AICL_BIT);
			if (rc < 0) {
				printk(KERN_ERR "opluschg_monitor_func: Couldn't set RESTART_AICL_BIT rc=%d\n", rc);
				goto rerun_work;
			}
			printk(KERN_ERR "opluschg_monitor_func: ichg[%d], fv[%d]\n", chip->icharging, opluschg_get_fv_monitor(chip));
		}
		counts = 0;
	}
...
static bool opluschg_usbtemp_condition(void)
{
	int rc;
	bool vbus_rising;
	u8 stat;
...
static void opluschg_usbtemp_dischg(void)
{
	int rc;
	struct smb_charger *chg = NULL;

	if (!g_oplus_chip) {
		chg_err("fail to init oplus_chip or adc_pinctrl\n");
		return;
	}

	chg_debug("dischg_enable\n");
	chg = &g_oplus_chip->pmic_spmi.smb5_chip->chg;
	chg->usb_status |= USB_HIGH_TEMP;
	...
	pinctrl_select_state(g_oplus_chip->normalchg_gpio.pinctrl, g_oplus_chip->normalchg_gpio.dischg_enable);
...
static int opluschg_usbtemp_monitor_main(void *data)
{
	int detect_interval, total_time, bat_temp, voltage1, voltage2;
	int resistance1, resistance2, temperature1, temperature2, index;
	...
	if (high_temp_version == true) {
		chg_debug("version not support: %d %d\n", cmp_pcb("EVT2"), high_temp_version);
		return 0;
	}

	detect_interval = DETECT_INTERVAL_50MS;

	while (!kthread_should_stop()) {
		if (opluschg_usbtemp_condition() == true) {
			voltage1 = 0;
			voltage2 = 0;
			for (index = 0; index < ADC_SAMPLE_COUNT; index ++) {
				voltage1 += opluschg_read_adc(USB_TEMPERATURE1);	/* PM7150A ADC */
				voltage2 += opluschg_read_adc(USB_TEMPERATURE2);	/* PM7150 ADC */
			...
			g_oplus_chip->usbtemp_volt_l = voltage1;
			g_oplus_chip->usbtemp_volt_r = voltage2;

			resistance1 = voltage_convert_resistance(voltage1, PULL_UP_VOLTAGE_1800MV, PULL_UP_RESISTANCE_51KOHM);
			resistance2 = voltage_convert_resistance(voltage2, PULL_UP_VOLTAGE_1800MV, PULL_UP_RESISTANCE_51KOHM);
			...
			add_temperature_list(temperature1, temperature2, detect_interval, index);
			check_temperature_list(temperature_list1, total_time, critical_rise1, index);
			check_temperature_list(temperature_list2, total_time, critical_rise2, index);

			if (oplus_warp_get_fastchg_started() == true)
				bat_temp = oplus_gauge_get_prev_batt_temperature();
			else
				bat_temp = oplus_gauge_get_batt_temperature();

			check_dischg_enable(temperature1, temperature2, critical_rise1, critical_rise2, bat_temp, dischg_enable);

			if (usbtemp_debug == 1)
				chg_debug("[%d %d][%d %d %d %d][%d %d %d %d]\n", bat_temp, dischg_enable, voltage1, resistance1,
			...
			wait_event_interruptible(opluschg_usbtemp_wq, opluschg_usbtemp_condition() == true);
			do_gettimeofday(&time2);
			if (time2.tv_sec - time1.tv_sec >= 15) {
				if (usbtemp_debug == 1)
					chg_debug("clear_temperature_list %d %d\n", time1.tv_sec, time2.tv_sec);
				clear_temperature_list(index);
			}
		}

		msleep(detect_interval - 25);
	}

	return 0;
}

static void opluschg_usbtemp_thread_init(void)
{
	oplushg_usbtemp_kthread = kthread_run(opluschg_usbtemp_monitor_main, 0, "usbtemp_kthread");
...
static int opluschg_parse_custom_dt(struct oplus_chg_chip *chip)
{
	...
	/* adc channel */
	smblib_get_iio_channel(chg, "chgid_voltage", &chg->iio.chgid_v_chan);
...
static void opluschg_set_chargerid_switch(int value)
{
	...
	if (oplus_warp_get_adapter_update_real_status() == ADAPTER_FW_NEED_UPDATE
...
static void opluschg_dump_regs(void)
{
	/* do nothing */
}

static int opluschg_kick_wdt(void)
{
	return 0;
}

static int opluschg_hw_init(void)
{
	int boot_mode = get_boot_mode();
	...
	opluschg_charge_enble();

	return 0;
}

static int opluschg_set_fcc(int current_ma)
{
	int rc = 0;
	struct oplus_chg_chip *chip = g_oplus_chip;
...
static void opluschg_set_aicl_point(int vol)
{
	/* do nothing */
}

static void opluschg_aicl_enable(bool enable)
{
	int rc = 0;
	u8 aicl_op;
	struct oplus_chg_chip *chip = g_oplus_chip;
...
static void opluschg_rerun_aicl(void)
{
	...
	msleep(50);
	opluschg_aicl_enable(true);
}

static bool  opluschg_is_normal_mode(void)
{
	int boot_mode = get_boot_mode();
...
static bool opluschg_is_suspend_status(void)
{
	int rc = 0;
	u8 stat;
	struct smb_charger *chg = NULL;
...
static void opluschg_clear_suspend(void)
{
	int rc;
	struct smb_charger *chg = NULL;
	int boot_mode = get_boot_mode();
...
static void opluschg_check_clear_suspend(void)
{
	use_present_status = true;
	opluschg_clear_suspend();
...
static int opluschg_set_icl(int current_ma)
{
	...
	chg_vol = opluschg_read_adc(USBIN_VOLTAGE);
	if (get_client_vote(chip->pmic_spmi.smb5_chip->chg.usb_icl_votable, BOOST_BACK_VOTER) == 0
		&& get_effective_result(chip->pmic_spmi.smb5_chip->chg.usb_icl_votable) <= USBIN_25MA) {
		chg_debug("use 500 here\n");
		goto aicl_boost_back;
	...
	if (opluschg_is_suspend_status() && opluschg_is_usb_present() && opluschg_is_normal_mode()) {
		i = i - 1;
		goto aicl_suspend;
	}
	chg_vol = opluschg_read_adc(USBIN_VOLTAGE);
...
static int opluschg_set_fv(int vfloat_mv)
{
	int rc = 0;
	struct oplus_chg_chip *chip = g_oplus_chip;
...
static int opluschg_set_iterm(int term_current)
{
	int rc = 0;
	u8 val_raw = 0;
	struct oplus_chg_chip *chip = g_oplus_chip;
...
static int opluschg_get_charge_enable(void)
{
	int rc = 0;
	u8 temp = 0;
	struct oplus_chg_chip *chip = g_oplus_chip;
...
static int opluschg_set_rechg_vol(int rechg_vol)
{
	return 0;
}

static int opluschg_reset_charge(void)
{
	return 0;
}

static int opluschg_read_full(void)
{
	int rc = 0;
	u8 stat = 0;
	struct oplus_chg_chip *chip = g_oplus_chip;

	if (!opluschg_is_usb_present())
		return 0;
...
static int opluschg_otg_enable(void)
{
	return 0;
}

static int opluschg_otg_disable(void)
{
	return 0;
}

static int opluschg_set_chging_term_disable(void)
{
	int rc;
	struct smb_charger *chg = NULL;
...
static bool opluschg_check_charger_resume(void)
{
	return true;
}

static bool opluschg_need_to_check_ibatt(void)
{
	return false;
}

static int opluschg_get_fcc_step(void)
{
	return 50;
}

static int opluschg_get_charger_volt(void)
{
	return opluschg_read_adc(USBIN_VOLTAGE);
}

static int opluschg_get_ibus(void)
{
	struct smb_charger *chg = NULL;
	union power_supply_propval val;
...
static bool opluschg_is_usb_present(void)
{
	...
	if (vbus_rising == false && oplus_warp_get_fastchg_started() == true) {
		if (opluschg_read_adc(USBIN_VOLTAGE) > 2000) {
			chg_err("USBIN_PLUGIN_RT_STS_BIT low but fastchg started true and chg vol > 2V\n");
			vbus_rising = true;
		}
	}

	if (vbus_rising == false && (oplus_warp_get_fastchg_started() == true && chip->vbatt_num == 2)) {
			chg_err("USBIN_PLUGIN_RT_STS_BIT low but fastchg started true and SWARP\n");
			vbus_rising = true;
	}
...
static int opluschg_get_shutdown_soc(void)
{
	return 0;
}

static int opluschg_backup_soc(int backup_soc)
{
	return 0;
}

static int opluschg_get_aicl_ma(void)
{
	return 0;
}

static int opluschg_force_tlim_en(bool enable)
{
	return 0;
}

static int opluschg_set_system_temp_level(int lvl_sel)
{
	return 0;
}

static int opluschg_set_prop_flash_active(enum skip_reason reason, bool disable)
{
	return 0;
}

static int opluschg_set_dpdm(int val)
{
	return 0;
}

static int opluschg_calc_flash_current(void)
{
	return 0;
}

static int opluschg_get_fv(struct oplus_chg_chip *chip)
{
...
static int opluschg_get_charging_current(struct oplus_chg_chip *chip)
{
...
static int opluschg_rtc_reset_check(void)
{
	struct rtc_time tm;
 ...
 */
static int opluschg_get_dyna_aicl_result(void)
{
...
static int opluschg_get_boot_mode(void)
{
	return get_boot_mode();
}

#ifdef CONFIG_OPLUS_SHORT_HW_CHECK
static bool opluschg_get_shortc_hw_gpio_status(void)
{
	return true;
}
#else
...
struct oplus_chg_operations opluschg_smb5_ops = {
	...
	.hardware_init			= opluschg_hw_init,
	.charging_current_write_fast	= opluschg_set_fcc,
	.set_aicl_point			= opluschg_set_aicl_point,
	.input_current_write		= opluschg_set_icl,
	.float_voltage_write		= opluschg_set_fv,
	.term_current_set		= opluschg_set_iterm,
	.charging_enable		= opluschg_charge_enble,
	.charging_disable		= opluschg_charge_disble,
	.get_charging_enable		= opluschg_get_charge_enable,
	.charger_suspend		= opluschg_suspend,
	.charger_unsuspend		= opluschg_unsuspend,
	.set_rechg_vol			= opluschg_set_rechg_vol,
	.reset_charger			= opluschg_reset_charge,
	.read_full			= opluschg_read_full,
	.otg_enable			= opluschg_otg_enable,
	.otg_disable			= opluschg_otg_disable,
	.set_charging_term_disable	= opluschg_set_chging_term_disable,
	.check_charger_resume		= opluschg_check_charger_resume,
	.get_chargerid_volt		= opluschg_get_chargerid,
	.set_chargerid_switch_val	= opluschg_set_chargerid_switch,
	.get_chargerid_switch_val	= opluschg_get_chargerid_switch,
	.need_to_check_ibatt		= opluschg_need_to_check_ibatt,
	.get_chg_current_step		= opluschg_get_fcc_step,
	.get_charger_type		= opluschg_get_charger_type,
	.get_charger_volt		= opluschg_get_charger_volt,
	.get_ibus			= opluschg_get_ibus,
	.check_chrdet_status		= opluschg_is_usb_present,
	.get_instant_vbatt		= opluschg_get_battery_voltage,
	.get_boot_mode			= opluschg_get_boot_mode,
	.get_boot_reason		= opluschg_get_boot_reason,
	.get_rtc_soc			= opluschg_get_shutdown_soc,
	.set_rtc_soc			= opluschg_backup_soc,
	.get_aicl_ma			= opluschg_get_aicl_ma,
	.rerun_aicl			= opluschg_rerun_aicl,
	.tlim_en			= opluschg_force_tlim_en,
	.set_system_temp_level		= opluschg_set_system_temp_level,
	.otg_pulse_skip_disable		= opluschg_set_prop_flash_active,
	.set_dp_dm			= opluschg_set_dpdm,
	.calc_flash_current		= opluschg_calc_flash_current,
#ifdef CONFIG_OPLUS_RTC_DET_SUPPORT
	.check_rtc_reset		= opluschg_rtc_reset_check,
#endif
#ifdef CONFIG_OPLUS_SHORT_C_BATT_CHECK
...
/* Yichun.Chen  PSW.BSP.CHG  2019-04-08  for charge */
	...
	oplus_chip->chg_ops = &opluschg_smb5_ops;
...
/* Yichun.Chen  PSW.BSP.CHG  2019- */
	...
	if (main_psy) {
		pval.intval = 1000 * opluschg_get_fv(oplus_chip);
		power_supply_set_property(main_psy,
				POWER_SUPPLY_PROP_VOLTAGE_MAX,
				&pval);
		pval.intval = 1000 * opluschg_get_charging_current(oplus_chip);
		power_supply_set_property(main_psy,
				POWER_SUPPLY_PROP_CONSTANT_CHARGE_CURRENT_MAX,
				&pval);
	}
...
#ifdef OPLUS_FEATURE_CHG_BASIC
/* Yichun.Chen  PSW.BSP.CHG  2019-04-08  for charge */
	opluschg_usbtemp_thread_init();
#endif

#ifdef OPLUS_FEATURE_CHG_BASIC
/* Yichun.Chen  PSW.BSP.CHG  2019-04-29  for charge debug work */
	INIT_DELAYED_WORK(&opluschg_debug_work, opluschg_debug_func);
	schedule_delayed_work(&opluschg_debug_work, 0);
#endif
...
/* Yichun.Chen  PSW.BSP.CHG  2019-04-25  for ship mode */
	if (g_oplus_chip && g_oplus_chip->enable_shipmode) {
		...
		mutex_lock(&g_oplus_chip->pmic_spmi.smb5_chip->chg.pinctrl_mutex);
		for (i = 0; i < 5; i ++) {
			pinctrl_select_state(g_oplus_chip->normalchg_gpio.pinctrl, g_oplus_chip->normalchg_gpio.ship_active);
			mdelay(3);
			pinctrl_select_state(g_oplus_chip->normalchg_gpio.pinctrl, g_oplus_chip->normalchg_gpio.ship_sleep);
			mdelay(3);
		}
		mutex_unlock(&g_oplus_chip->pmic_spmi.smb5_chip->chg.pinctrl_mutex);
	}
...
static int opluschg_get_current_time(unsigned long *now_tm_sec)
{
	struct rtc_time tm;
	struct rtc_device *rtc;
	int rc;
...
static int opluschg_pm_resume(struct device *dev)
{
	int rc = 0;
	...
	rc = opluschg_get_current_time(&resume_tm_sec);
...
static int opluschg_pm_suspend(struct device *dev)
{
	if (!g_oplus_chip)
		return 0;

	if (opluschg_get_current_time(&suspend_tm_sec)) {
		chg_err("RTC read failed\n");
		suspend_tm_sec = -1;
	}
...
static const struct dev_pm_ops smb5_pm_ops = {
	.resume		= opluschg_pm_resume,
	.suspend	= opluschg_pm_suspend,
};
#endif
